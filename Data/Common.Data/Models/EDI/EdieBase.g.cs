//------------------------------------------------------------------------------
// 
//     This code was generated by a tool.
//     Template version $Rev: 12700 $
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
//
//     (c) Infor Global Solutions 2018
// 
//------------------------------------------------------------------------------

#pragma warning disable 1591
using System;
using System.ComponentModel.DataAnnotations;
using System.Data;
using ServiceInterfaceClient.Attributes;
using ServiceInterfaceClient.BaseClasses;
using ServiceInterfaceClient.Helpers;
using ServiceInterfaceClient.Interfaces;
       
namespace Infor.Sxe.Common.Data.Models.EDI
{
   /// <summary>
   /// EDI Error File.  Used to store error records associated to EDIH and EDIL records, based upon errors found EDI Data Translated.
   /// </summary>
   
   public partial class EdieBase: ModelBase, IUserFields
   {

      /// <summary>
      /// Company Number
      /// </summary>
      [Key,Order]
      public int cono { get; set; }

      /// <summary>
      /// Batch Name
      /// </summary>
      [Key,Order,StringValidationAttribute]
      public string batchnm { get; set; }

      /// <summary>
      /// Seq #
      /// </summary>
      [Key,Order]
      public int seqno { get; set; }

      /// <summary>
      /// Level
      /// </summary>
      [Key,Order]
      public bool level { get; set; }

      /// <summary>
      /// Line #
      /// </summary>
      [Key,Order]
      public int lineno { get; set; }

      /// <summary>
      /// Err Seq #
      /// </summary>
      [Key,Order]
      public int errseqno { get; set; }

      /// <summary>
      /// Doc Type
      /// </summary>
      [Key,Order,StringValidationAttribute]
      public string docty { get; set; }

      /// <summary>
      /// Field Type
      /// </summary>
      [StringValidationAttribute]
      public string fieldty { get; set; }

      /// <summary>
      /// Field Value
      /// </summary>
      [StringValidationAttribute]
      public string fieldvalue { get; set; }

      /// <summary>
      /// Err Type
      /// </summary>
      [StringValidationAttribute]
      public string errty { get; set; }

      /// <summary>
      /// statusty
      /// </summary>
      [StringValidationAttribute]
      public string statusty { get; set; }

      /// <summary>
      /// Last Change By
      /// </summary>
      [StringValidationAttribute]
      public string operinit { get; set; }

      /// <summary>
      /// Last Change Date
      /// </summary>
      public DateTime? transdt { get; set; }

      /// <summary>
      /// Last Change Time
      /// </summary>
      [StringValidationAttribute]
      public string transtm { get; set; }

      /// <summary>
      /// Trans Proc
      /// </summary>
      [StringValidationAttribute]
      public string transproc { get; set; }

      /// <summary>
      /// User 1
      /// </summary>
      [StringValidationAttribute]
      public string user1 { get; set; }

      /// <summary>
      /// User 2
      /// </summary>
      [StringValidationAttribute]
      public string user2 { get; set; }

      /// <summary>
      /// User 3
      /// </summary>
      [StringValidationAttribute]
      public string user3 { get; set; }

      /// <summary>
      /// User 4
      /// </summary>
      [StringValidationAttribute]
      public string user4 { get; set; }

      /// <summary>
      /// User 5
      /// </summary>
      [StringValidationAttribute]
      public string user5 { get; set; }

      /// <summary>
      /// User 6
      /// </summary>
      public decimal? user6 { get; set; }

      /// <summary>
      /// User 7
      /// </summary>
      public decimal? user7 { get; set; }

      /// <summary>
      /// User 8
      /// </summary>
      public DateTime? user8 { get; set; }

      /// <summary>
      /// User 9
      /// </summary>
      public DateTime? user9 { get; set; }

      /// <summary>
      /// Default Whse
      /// </summary>
      [StringValidationAttribute]
      public string whse { get; set; }

      /// <summary>
      /// Customer #
      /// </summary>
      public decimal custno { get; set; }

      /// <summary>
      /// Ship To
      /// </summary>
      [StringValidationAttribute]
      public string shipto { get; set; }

      /// <summary>
      /// Original Field Value
      /// </summary>
      [StringValidationAttribute]
      public string fieldvalueorig { get; set; }

      /// <summary>
      /// Data Identifier
      /// </summary>
      [StringValidationAttribute]
      public string dataid { get; set; }

      /// <summary>
      /// EDI Ln#
      /// </summary>
      [StringValidationAttribute]
      public string edilineno { get; set; }
      
      /// <summary>
      /// Row ID
      /// </summary>
      [StringValidationAttribute]
      public string rowID { get; set; }



      /// <summary>
      /// Build a class from a database row
      /// </summary>
      public static T BuildEdieBaseFromRow<T>(DataRow row) where T:EdieBase, new()
      {
         T entity = new T();
         entity.cono = row.IsNull("cono") ? 0 : row.Field<int>("cono");
         entity.batchnm = row.IsNull("batchnm") ? string.Empty : row.Field<string>("batchnm");
         entity.seqno = row.IsNull("seqno") ? 0 : row.Field<int>("seqno");
         entity.level = row.Field<bool>("level");
         entity.lineno = row.IsNull("lineno") ? 0 : row.Field<int>("lineno");
         entity.errseqno = row.IsNull("errseqno") ? 0 : row.Field<int>("errseqno");
         entity.docty = row.IsNull("docty") ? string.Empty : row.Field<string>("docty");
         entity.fieldty = row.IsNull("fieldty") ? string.Empty : row.Field<string>("fieldty");
         entity.fieldvalue = row.IsNull("fieldvalue") ? string.Empty : row.Field<string>("fieldvalue");
         entity.errty = row.IsNull("errty") ? string.Empty : row.Field<string>("errty");
         entity.statusty = row.IsNull("statusty") ? string.Empty : row.Field<string>("statusty");
         entity.operinit = row.IsNull("operinit") ? string.Empty : row.Field<string>("operinit");
         entity.transdt = row.Field<DateTime?>("transdt");
         entity.transtm = row.IsNull("transtm") ? string.Empty : row.Field<string>("transtm");
         entity.transproc = row.IsNull("transproc") ? string.Empty : row.Field<string>("transproc");
         entity.user1 = row.IsNull("user1") ? string.Empty : row.Field<string>("user1");
         entity.user2 = row.IsNull("user2") ? string.Empty : row.Field<string>("user2");
         entity.user3 = row.IsNull("user3") ? string.Empty : row.Field<string>("user3");
         entity.user4 = row.IsNull("user4") ? string.Empty : row.Field<string>("user4");
         entity.user5 = row.IsNull("user5") ? string.Empty : row.Field<string>("user5");
         entity.user6 = row.Field<decimal?>("user6");
         entity.user7 = row.Field<decimal?>("user7");
         entity.user8 = row.Field<DateTime?>("user8");
         entity.user9 = row.Field<DateTime?>("user9");
         entity.whse = row.IsNull("whse") ? string.Empty : row.Field<string>("whse");
         entity.custno = row.IsNull("custno") ? decimal.Zero : row.Field<decimal>("custno");
         entity.shipto = row.IsNull("shipto") ? string.Empty : row.Field<string>("shipto");
         entity.fieldvalueorig = row.IsNull("fieldvalueorig") ? string.Empty : row.Field<string>("fieldvalueorig");
         entity.dataid = row.IsNull("dataid") ? string.Empty : row.Field<string>("dataid");
         entity.edilineno = row.IsNull("edilineno") ? string.Empty : row.Field<string>("edilineno");
         entity.rowID = row.Field<byte[]>("edieRowID").ToStringEncoded();
         return entity;
      }

      /// <summary>
      /// Update a database row from a class
      /// </summary>
      public static void UpdateRowFromEdieBase(ref DataRow row, EdieBase entity)
      {
         row.SetField("cono", entity.cono);
         row.SetField("batchnm", entity.batchnm);
         row.SetField("seqno", entity.seqno);
         row.SetField("level", entity.level);
         row.SetField("lineno", entity.lineno);
         row.SetField("errseqno", entity.errseqno);
         row.SetField("docty", entity.docty);
         row.SetField("fieldty", entity.fieldty);
         row.SetField("fieldvalue", entity.fieldvalue);
         row.SetField("errty", entity.errty);
         row.SetField("statusty", entity.statusty);
         row.SetField("operinit", entity.operinit);
         row.SetField("transdt", entity.transdt);
         row.SetField("transtm", entity.transtm);
         row.SetField("transproc", entity.transproc);
         row.SetField("user1", entity.user1);
         row.SetField("user2", entity.user2);
         row.SetField("user3", entity.user3);
         row.SetField("user4", entity.user4);
         row.SetField("user5", entity.user5);
         row.SetField("user6", entity.user6);
         row.SetField("user7", entity.user7);
         row.SetField("user8", entity.user8);
         row.SetField("user9", entity.user9);
         row.SetField("whse", entity.whse);
         row.SetField("custno", entity.custno);
         row.SetField("shipto", entity.shipto);
         row.SetField("fieldvalueorig", entity.fieldvalueorig);
         row.SetField("dataid", entity.dataid);
         row.SetField("edilineno", entity.edilineno);
         row.SetField("edieRowID", entity.rowID.ToByteArray());
      }   
      
      /// <summary>
      /// Build a minimal row from a class (key fields only)
      /// </summary>
      public static void BuildMinimalRow(ref DataRow row, EdieBase entity)
      {
         row.SetField("cono", entity.cono);
         row.SetField("batchnm", entity.batchnm);
         row.SetField("seqno", entity.seqno);
         row.SetField("level", entity.level);
         row.SetField("lineno", entity.lineno);
         row.SetField("errseqno", entity.errseqno);
         row.SetField("docty", entity.docty);
         row.SetField("edieRowID", entity.rowID.ToByteArray());
      }   
   }
}
#pragma warning restore 1591
	