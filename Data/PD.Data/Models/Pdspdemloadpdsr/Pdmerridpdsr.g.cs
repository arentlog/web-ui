//------------------------------------------------------------------------------
// 
//     This code was generated by a tool.
//     Template version $Rev: 13370 $
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
//
//     (c) Infor Global Solutions 2018
// 
//------------------------------------------------------------------------------

#pragma warning disable 1591
using System;
using System.ComponentModel.DataAnnotations;
using System.Data;
using System.Collections.Generic;
using System.Linq;
using HelpPageErrorSimulator.HelpArea.Areas.HelpPage.ModelDescriptions;
using ServiceInterfaceClient.Attributes;
using ServiceInterfaceClient.BaseClasses;
using ServiceInterfaceClient.Helpers;
using ServiceInterfaceClient.Interfaces;

namespace Infor.Sxe.PD.Data.Models.Pdspdemloadpdsr
{
   [ModelName("Infor.Sxe.PD.Data.Models.Pdspdemloadpdsr.Pdmerridpdsr")]
   public partial class Pdmerridpdsr : ModelBase
   {
      [StringValidationAttribute]
      public string pdRowid { get; set; }
      [StringValidationAttribute]
      public string cErrorMsg { get; set; }
      [StringValidationAttribute]
      public string userfield { get; set; }


      public static Pdmerridpdsr BuildPdmerridpdsrFromRow(DataRow row)
      {
         Pdmerridpdsr entity = new Pdmerridpdsr();
         entity.pdRowid = row.Field<byte[]>("pdRowid").ToStringEncoded();
         entity.cErrorMsg = row.IsNull("cErrorMsg") ? string.Empty : row.Field<string>("cErrorMsg");
         entity.userfield = row.IsNull("userfield") ? string.Empty : row.Field<string>("userfield");
         return entity;
      }

      /// <summary>
      /// Update a database row from a class
      /// </summary>
      public static void UpdateRowFromPdmerridpdsr(ref DataRow row, Pdmerridpdsr entity)
      {
         row.SetField("pdRowid", entity.pdRowid.ToByteArray());
         row.SetField("cErrorMsg", entity.cErrorMsg);
         row.SetField("userfield", entity.userfield);

      }
   
   }
}
#pragma warning restore 1591
