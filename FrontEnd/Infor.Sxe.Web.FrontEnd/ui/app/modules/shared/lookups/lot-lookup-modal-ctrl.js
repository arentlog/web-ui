...
/**
 * Lot Lookup controller for the modal content.
 *
 * Alias: lkupmdl
 */
app.controller('LotLookupModalCtrl', function ($scope, Utils) {
   var self = this;
   var lkup = $scope.lkup;
   self.isWarehouseEnabled = true;
   self.isProductEnabled = true;
   self.isStatusTypeEnabled = true;
   ...
   self.initializeCriteria = function (isClear) {
      ...
      if (self.criteria && self.criteria.ourproc) {
         if (self.criteria.ourproc.toLowerCase() === 'kpew') {
            ...
            self.isStatusTypeEnabled = false;
         }
      }
